...
from Burn_Scar_Detection_Utils import validate_FIRMS, get_nearest_town, spatially_associate_arson_events
from shapely.geometry import Point


# Use absolute paths
BASE_DIR = r"C:\Users\tarig\Desktop\Arson_Analyser_Latest_Version"
FIRMS_DATA_PATH = os.path.join(BASE_DIR, "Data", "data.csv")
URBAN_FILTER_PATH = os.path.join(BASE_DIR, "Data", "Sudan_Filterer_Cleaned_Dissolved.gpkg")
SETTLEMENTS_PATH = os.path.join(BASE_DIR, "Data", "settlements.gpkg")
JOIN_PATH = os.path.join(BASE_DIR, "Data", "gadm41_SDN_3.shp")
OUTPATH_POLYGONS = os.path.join(BASE_DIR, "Outputs", "Validated_FIRMS_Detections_Town_Polygons.geojson")
OUTPATH_POINTS = os.path.join(BASE_DIR, "Outputs", "Validated_FIRMS_Detections_Points.geojson")
...
firms_data_df['geometry'] = gpd.points_from_xy(firms_data_df.longitude, firms_data_df.latitude)
...
print(f'A total of {len(urban_firms_detections)} fires detected by FIRMS in urban areas.')
...
ee.Authenticate()
ee.Initialize(project = "arson-analyser-423823")
...
def arson_analyser(urban_firms_detections_gdf):
    ...
    for i in tqdm(range(len(firms_detections))):
        ...
        if data_dict['no_data'] == False:
            firms_detections.loc[i, 'burn_scar_detected'] = data_dict['burn_scar_detected']
            if data_dict['burn_scar_detected'] == True:
                firms_detections.loc[i, 'burnt_pixel_count'] =  data_dict['burnt_pixel_count']
                firms_detections.loc[i, 'num_burnt_buildings'] =  data_dict['num_burnt_buildings']
        else:
            firms_detections.loc[i, 'no_data_reason'] = data_dict['no_data_reason']
    firms_detections = spatially_associate_arson_events(firms_detections) #new function to associate detections within the same town
...
validated_firms_polygons, validated_firms_detections_points = arson_analyser(urban_firms_detections)


print("Performing spatial join...")
validated_firms_detections_join = gpd.sjoin(validated_firms_detections_points, join_gdf, how='left', predicate='intersects')

# Specify the columns to include in the final CSV
columns_to_include = [
    'country_id', 'NAME_1', 'NAME_2', 'NAME_3', 'urban_area_name',
    'latitude', 'longitude', 'acq_date', 'unique_event_no', 'no_data',
    'no_data_reason', 'burn_scar_detected'
]

# Drop the geometry column and select only the specified columns
validated_firms_detections_final = validated_firms_detections_join.drop(columns='geometry', axis=1)[columns_to_include]
...
def generate_links(validated_firms_detections_final):
    for index, row in validated_firms_detections_final.iterrows():
        ...
        def generate_social_links(keyword):
            ...
            outer_json = {
                "rp_creation_time": inner_json_str
            }
        ...
        if pd.notna(row['urban_area_name']):
            pau_twitter_link, pau_whopostedwhat_link = generate_social_links(row['urban_area_name'])
            validated_firms_detections_final.at[index, 'vil_Twitter_Link'] = pau_twitter_link
            validated_firms_detections_final.at[index, 'vil_Whopostedwhat_Link'] = pau_whopostedwhat_link
        else:
        ...
        if pd.notna(row['NAME_2']):
            au_twitter_link, au_whopostedwhat_link = generate_social_links(row['NAME_2'])
            validated_firms_detections_final.at[index, 'ADM2_Twitter_Link'] = au_twitter_link
            validated_firms_detections_final.at[index, 'ADM2_Whopostedwhat_Link'] = au_whopostedwhat_link
        else:
        ...
        if pd.notna(row['NAME_3']):
            loc_twitter_link, loc_whopostedwhat_link = generate_social_links(row['NAME_3'])
            validated_firms_detections_final.at[index, 'ADM1_Twitter_Link'] = loc_twitter_link
            validated_firms_detections_final.at[index, 'ADM1_Whopostedwhat_Link'] = loc_whopostedwhat_link
        else:
...
validated_firms_detections_final.to_csv(OUTPATH_CSV, index=False, encoding='utf-8-sig', lineterminator='\n')
